...
| 5 | The pipeline does not generalize well to a client's data shape it has not seen before | Technical | 2 | 3 | Round 2 tests this directly against three different shapes: Olist (clean relational), CFPB (messy, single flat table, at full scale), and Open Food Facts (genuinely nested JSON, converted into relational tables) |
